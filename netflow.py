...
class Database:
  ...
  def insert(self, table, key=None, data):
    if table == "H":
      ...
      try:   
        self.data[table][key].update(key, data)
      except KeyError:
...
class RecordInfo(NetFlowInfo):
  def __init__(self, dataGroup):  
    self.nextHop = dataGroup[HOP_IP-2]
    self.snmpIn = dataGroup[IF_IN-2]
    self.snmpOut = dataGroup[IF_OUT-2]
    self.numPkts = dataGroup[NUM_PKTS-2]
    self.L3Bytes = dataGroup[L3_BYTES-2]
    self.flowStart = dataGroup[START-2]
    self.flowStop = dataGroup[END-2]
    self.srcPort = dataGroup[SRC_PORT-2]
    self.dstPort = dataGroup[DST_PORT-2]
    self.tcpFlags = dataGroup[TCP_FLAGS-2]
    self.ipProt = dataGroup[IP_PROT-2]
    self.tos = dataGroup[SRV_TYPE-2]
    self.srcAS = dataGroup[SRC_AS-2]
    self.dstAS = dataGroup[DST_AS-2] 
    self.srcMask = dataGroup[SRC_MASK-2]
    self.dstMask = dataGroup[DST_MASK-2]

  def update(self, key, dataGroup):
    print "Got another record for: "+str(key) 
